library(shiny)
library(forecast) 
data1 <- read.csv("inbdkorea.csv", header = TRUE, sep = ",")
datats <- ts(data1, frequency=12, start=c(2010,1))

DData<-data.frame(Time=seq(1,120),datats)
shinyServer(function(input,output,session){
  ...
  output$PlotG<-renderPlot({
    if(is.null(MyData())!=T){
      plot(MyData()[,Col()],ylab="A number of Korea Inbound Tourists")+lines(MyData()[,Col()])
    }
  })
  output$Plot1<-renderPlot({
    ...
    if(input$F2==1){
      ...
      plot(forecast1,xlab="Time",ylab="A number of Korea Inbound Tourists")
  ...
  output$Plot3<-renderPlot({
    ...
    if(input$F4==1){
      ...
      plot(forecast,xlab="Time",ylab="A number of Korea Inbound Tourists")
